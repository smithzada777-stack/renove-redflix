        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" width="100%" class="container" style="background-color: #ffffff;">
            ...
            <tr>
                <td align="center" style="padding: 25px; background-color: #fafafa; color: #999999; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; border-top: 1px solid #eeeeee;">
                    <p style="margin: 0;">RedFlix © 2026 • Entretenimento em Alta Definição</p>
                </td>
            </tr>
        </table>
    </center>
...
async function sendGboxTests() {
    console.log(`--- ENVIANDO TESTES COMPATÍVEIS GMAIL PARA: ${targetEmail} ---`);
    const supportPhone = '5571991644164';

    // 1. Teste PENDENTE (Site)
    const plan1 = 'Plano VIP Mensal';
    const waMsgPending = encodeURIComponent(`Olá, acabei de assinar o plano ${plan1} na RedFlix e tenho dúvidas sobre o pagamento, poderia me ajudar?`);

    const contentPending = `
        <div style="font-size: 40px; margin-bottom: 10px;">⏳</div>
        <h2 style="font-size: 26px; font-weight: 900; margin: 0 0 10px 0; color: #111111; letter-spacing: -1px;">PEDIDO REALIZADO</h2>
        <p style="font-size: 16px; line-height: 1.6; margin: 0 10%; color: #666666;">
            Recebemos sua solicitação para o plano <strong>${plan1}</strong>.
        </p>

        <div style="background-color: #f9f9f9; border: 1px solid #eeeeee; border-radius: 12px; padding: 25px; margin: 30px 0;">
            <p style="color: #999999; font-size: 10px; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; margin: 0 0 5px 0;">VALOR DO INVESTIMENTO</p>
            <p style="color: #E50914; font-size: 38px; font-weight: 900; margin: 0; letter-spacing: -1px;">R$ 29,90</p>
        </div>

        <p style="font-size: 14px; color: #777; margin-bottom: 10px;">Se você já pagou ou precisa de ajuda:</p>
        <a href="https://wa.me/${supportPhone}?text=${waMsgPending}" class="button">
            FALAR COM SUPORTE
        </a>
    `;

    // 2. Teste APROVADO (RedFlix)
    const plan2 = 'Plano VIP Semestral';
    const waMsgApproved = encodeURIComponent(`Oi, acabei de assinar meu plano ${plan2} na RedFlix e ainda não recebi meu acesso, poderia me ajudar?`);
    ...
    const contentApproved = `
        ...
        <p style="font-size: 16px; line-height: 1.6; margin: 0 10%; color: #666666;">
            Seu plano <strong>${plan2}</strong> foi ativado. Sua diversão começa agora!
        </p>
        ...
        <div style="background-color: #f9f9f9; border: 1px solid #eeeeee; border-radius: 12px; padding: 25px; text-align: left; margin: 30px 0;">
            ...
            <div style="margin-bottom: 10px; font-size: 14px; color: #333;"><strong>1.</strong> Clique no botão de suporte abaixo.</div>
            <div style="margin-bottom: 10px; font-size: 14px; color: #333;"><strong>2.</strong> Informe que assinou pela <strong>RedFlix</strong>.</div>
            <div style="font-size: 14px; color: #333;"><strong>3.</strong> Nossa equipe enviará seu login e senha.</div>
        </div>
