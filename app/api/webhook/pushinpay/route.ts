...
export async function POST(req: Request) {
    try {
        ...
        console.log('--- [WEBHOOK PUSHINPAY RENOVE] RECEBIDO ---');
        ...
        const positiveStatuses = ['paid', 'approved', 'confirmed', 'concluido', 'sucesso', '1'];
            ...
            if (adminDb) {
                ...
                if (!snapshot.empty) {
                    for (const doc of snapshot.docs) {
                        ...
                        try {
                            const newStatus = leadData.isRenewal || leadData.origin === 'renove' ? 'renewed' : 'approved';
